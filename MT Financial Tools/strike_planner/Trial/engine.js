// ==========================================
        ...
        auth.onAuthStateChanged((user) => {
            const cloudBtn = document.getElementById('btn-cloud-load');
            if (user) {
                currentUser = user;
                if (cloudBtn) {
                    cloudBtn.innerText = "Load Cloud Profile";
                    cloudBtn.onclick = loadCloudProfile;
                }
            } else {
                currentUser = null;
                if (cloudBtn) {
                    cloudBtn.innerText = "Load Save File";
                    cloudBtn.onclick = () => document.getElementById('file-upload').click();
                }
            }
        });

        async function loadCloudProfile() {
            if (!currentUser) return alert("You must be logged in to load a cloud profile.");
            
            const cloudBtn = document.getElementById('btn-cloud-load');
            const originalText = cloudBtn.innerText;
            
            // Visual feedback
            cloudBtn.innerText = "Loading Cloud Data...";
            cloudBtn.disabled = true;

            try {
                // Fetch user document from 'users' collection using their UID
                const docRef = db.collection('users').doc(currentUser.uid);
                const docSnap = await docRef.get();
                
                if (docSnap.exists) {
                    const data = docSnap.data();
                    // Assumes your JSON state is saved exactly as the document object, 
                    // or nested inside a 'state' object field.
                    ...
                    alert("No cloud profile found. Please start fresh or load a local file.");
                }
            } catch (error) {
                console.error("Error loading cloud data:", error);
                alert("Failed to load cloud profile. Check console for details.");
            } finally {
                cloudBtn.innerText = originalText;
            ...
            const imgBlockHTML = `
                <div class="w-full mt-4 pt-4 border-t border-slate-200 dark:border-slate-700/50">
                    ...
                    <div id="paste-area" class="w-full aspect-video bg-white dark:bg-slate-900/30 rounded-2xl border-2 border-dashed border-slate-300 dark:border-slate-600 hover:border-brand transition-colors flex items-center justify-center relative overflow-hidden group shadow-inner">
                        <div class="text-center placeholder-text ${s.image ? 'hidden' : ''}" id="img-placeholder" onclick="document.getElementById('chart-upload').click()" style="cursor:pointer;">
                            ...
                            <p class="text-base text-slate-500 font-bold px-4">Tap to Upload or Paste <kbd class="hidden sm:inline-block px-2 py-1 bg-slate-100 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded font-mono text-sm">Ctrl+V</kbd></p>
                        </div>
        ...
        // V1.8 Helper: Processes the file input from the Technical Chart placeholder
...
R:R: ${rr.toFixed(2)}`;
        ...
        function saveData() { 
            ...
            a.href = url; a.download = `MT_StrikePlanner_v1.8.json`; a.click(); URL.revokeObjectURL(url); 
        }